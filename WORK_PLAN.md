...
- [ ] **P6.22** — **KW12 gap G1: add `ProductionOrders` import target.** Needed to auto-import textile weekly files (Matriks sheet). Source: `docs/KW12_ImportAnalysis.md` §3. Groups rows by WorkOrder root → creates parent + per-size variant children + ProductionOrderMaterial per row.
- [ ] **P6.23** — **KW12 gap G2: `CustomsDeclarationLine.IsPreferentialOrigin` column + migration.** Faktura `UL` values like `BG no pref.` encode preferential origin flag for duty calculation; today we strip the suffix via CSV preprocess and lose the fidelity.
- [ ] **P6.24** — **KW12 gap G3: `ProductionOrderMaterial.PreAssignedMRN/BatchNumber` columns.** Textile partners pre-assign which MRN each WO line consumes; LON FEFO-picks instead. Needs respecting pre-assignment in `CreateMaterialIssueCommand`.
- [ ] **P6.25** — **KW12 gap G7: legacy items soft-delete problem.** Phase-3 migration left ~half of KW12 ArtNrs in `IsDeleted=1`. Options: (a) backfill `UPDATE Items SET IsDeleted=0` on VPS, (b) add "skip-if-exists" mode to the Items import executor, (c) fix the legacy migration. Empirical impact: 121/259 KW12 items blocked Faktura `itemCode` LOOKUP until manual undelete.
- [ ] **P6.26** — **KW12 gap G8: `POST /api/masterdata/uom` always saves `IsDeleted=true`.** Controller ignores `request.IsActive`. One-line fix. Also applies to any other MasterData POST endpoint that shares the shape.
- [ ] **P6.27** — **KW12 gap G9: CustomsDeclarations import executor missing MRN uniqueness pre-check.** Dry-run passes, commit throws SQL 2601 on `(TenantId, MRN)` unique index. Add `Items.AnyAsync(d => d.MRN == mrn)` pre-check next to the DeclarationNumber check.
- [ ] **P6.28** — **KW12 soft gaps S1–S3, S5, S7** (bundle one migration): `ProductionOrder.CustomerOrderNumber`, `ProductionOrder.CustomerPartnerId`, `ProductionOrder.WeekNumber`, `CustomsDeclaration.CMRNumber + ClosingNumber + CommercialInvoiceNumber`, `BOMLine.EfficiencyFactor`, `CustomsDeclaration.TotalGrossWeight + TotalNetWeight`.
