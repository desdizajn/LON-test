...
- [x] 17.12  SQL SEQUENCE objects + NumberFormatter — deployed `81269ba`; per-tenant `seq_Receipt_*` + `seq_Shipment_*` + `seq_MaterialIssue_*` + `seq_ProductionOrder_*` created on VPS; 4 handlers refactored from guid-suffix to NumberFormatter+SEQUENCE.  → BLUEPRINT §6.6
- [x] 17.13  Audit interceptor + AuditLogEntry writes + /admin/audit-log UI activation — deployed `ebd2fce`; capture logic already in `ApplicationDbContext.SaveChangesAsync`; new `AuditHistoryTab` on ClientOrder hub + 3 integration tests; VPS smoke shows Create + Update + Status-change entries with field-level diffs.  → BLUEPRINT §6.5
- [x] 17.14  Soft-delete global filter + recycle bin UI (block-delete with children) — deployed `1b884a0`; ClientOrder now ISoftDeletable; Cancel handler refuses if non-deleted children exist; `/admin/recycle-bin` UI with Restore + Permanent-delete; SoftDeleteRetentionJob worker (90d). VPS smoke: cancel + bin list + restore round-trip ✅.  → BLUEPRINT §6.7
