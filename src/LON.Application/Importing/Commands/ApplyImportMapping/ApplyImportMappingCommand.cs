    ...
    : ICommandHandler<ApplyImportMappingCommand, Result<Guid>>
{
    ...
    public ApplyImportMappingCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }
        ...
        ApplyImportMappingCommand request, CancellationToken cancellationToken)
    {
        ...
        // Validate mapping against the uploaded headers — every non-ignored
        // column must reference a real header from the file.
        ...
        foreach (var col in request.Mapping.Columns)
        {
            ...
            if (!col.Ignore && string.IsNullOrWhiteSpace(col.TargetField))
