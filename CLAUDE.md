...
- **EF migrations:** 54 applied (последно: Phase 17 §E9 `P17_E9_AddRazdolzenaDaNeToDeclarationLine` — RazdolzenaDaNe/At/By columns on CustomsDeclarationLine). Recreate count со `ls src/LON.Infrastructure/Migrations/*.cs | grep -v Designer | grep -v ModelSnapshot | wc -l`.
...
*Последна ревизија: 2026-05-14 — **Phase 17 ИСТИНСКИ ЗАВРШЕНА**. После првичен синтетски close-out (кој не го задоволуваше CLAUDE.md §11.3 + PLAN.md §5), Z2779 е миграциран на VPS LONDB преку SSH тунел: 6/6 R-queries PASS, CO `CO-2025-000001 (O1-Z2779)` Status=Closed со 8 declaration lines, 13 InventoryMovements, 5 BOMLines, 5 MaterialIssues, 1 DeliveryNote, 77M MKD bond. Откриени и поправени 2 buga (ClientOrderSummaryDto missing reference; razdolzuvanje query на Closed migrated orders). Playwright 4/4 green против VPS таргетирајќи го реалниот Z2779 CO. Phase 17 acceptance criteria сите вистински check. EF migrations: 59. Next: Phase 18 (Subcontractor) + Phase 19 (Speditor) во паралел, потоа Phase 20 (RLS) → Phase 21 (cutover).*